...
var apiKey = "AIzaSyBix_tHvGMZgFLvZgcxztH8IKDIhr4z188"
var videoId = "_1R7ceseZEM";
...
fs.readFile('client_secret.json', function processClientSecrets(err, content) {
  ...
  //LiveChatVideoIdは配信毎に変化するので都度取得してくる
  //TODO　返り値もらいたいけどどうやるんだーい
  var liveChatId =  authorize(JSON.parse(content), getLiveChatId);

  
  var sleep = require('sleep-async')();
  //TODO　メッセージをポーリングして取得してきたいがWhileすると取得できない
  //while(true){
    //sleep.sleep(1000, function(){
    //TODO　返り値返す
    var messages = authorize(JSON.parse(content), getLiveChatMessage);
    //});
...
async　function getLiveChatMessage(auth)
{
  ...
  var pageToken = '';
      service.liveChatMessages.list({
        ...
        liveChatId: "EiEKGFVDeEZWU2ZTblB3Qnl0WFNpZG1NbmlkdxIFL2xpdmU",
        pageToken: pageToken,
      }, function(err, response) {
        if (err) {
          console.log('The API returned an error: ' + err);
